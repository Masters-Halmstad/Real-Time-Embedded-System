...
static thread dequeueItem(thread *queue, int idx)
{
	...
	{ // If the queue is not empty
		...
		{ // If the current thread is not the one to be removed
			...
			{							 // While the current thread is not the one to be removed
				Previous = Current;		 // Set the previous thread as the current thread
				Current = Current->next; // Set the next thread as the current thread
			}
			if (Previous != NULL)
			{ // If the previous thread is not NULL
...
static void sortX(thread *queue)
{
	...
	int n = 0;			 // Initialize the number of threads in the queue
	thread curr, high_p; // Initialize the current thread and the thread with the highest priority
	for (thread t = *queue; t->next != NULL; t = t->next)
	{		 // For each thread in the queue
		n++; // Increment the number of threads in the queue
	}
	int count = n;
	for (int i = 0; i <= n; i++)
	{ // For each thread in the queue
		curr = *queue;
		high_p = NULL; //	Set the thread with the highest priority as NULL
		...
		{ // For each thread in the queue
			...
			curr = curr->next; // Set the next thread as the current thread
		}
...
void respawn_periodic_tasks(void)
{
	...
	// restoring the thread unti currentT is not null
	while (currentT != NULL)
	{
