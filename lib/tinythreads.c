...
static void dispatch(thread next)
{
	...
	if (next != NULL)
	{
		...
		else // ensuring the current always stays occupied
		{
			current = next;
			longjmp(next->context, 1);
		}
...
void spawnWithDeadline(void (*function)(int), int arg, unsigned int deadline, unsigned int rel_deadline)
{
	...
	// Addind two new deadlines
	newp->Period_Deadline = deadline;		  // absoulte/first deadline
	newp->Rel_Period_Deadline = rel_deadline; // relative deadline
	...
	if (setjmp(newp->context) == 1)
	{
		...
		current->function(current->arg);
		DISABLE();
		enqueue(current, &doneQ); // restoring the tread to doneQ instead of freeQ
		current = NULL;
		dispatch(dequeue(&readyQ));
	}
	SETSTACK(&newp->context, &newp->stack);
...
static thread dequeueItem(thread *queue, int idx)
{
	...
	// Check if the queue is empty , return null if it is else performing dequingItem
	if (*queue != NULL)
	{
		// If the current thread is the one to be removed
		if (Current->idx == idx)
		{
			*queue = Current->next;
			return Current;
		...
		{ // If the current thread is not the one to be removed
			while (Current->idx != idx)
			{
				Previous = Current;		 // Setting the previous thread as the current thread
				Current = Current->next; // Setting the next thread as the current thread
			}
			if (Previous != NULL)
			{
...
{
	DISABLE();
	int n = 0;			 // Initializing the number of threads in the queue
	thread curr, high_p; // Initializing the current thread and the thread with the highest priority

	// Calculating the number of threads in the queue
	...
	{
		curr = *queue;
		high_p = NULL; //	Setting the thread with the highest priority as NULL
		...
		for (int j = 0; j <= count; j++)
		{
			...
			if (high_p == NULL)
			{
				high_p = curr; //	Setting the thread with the highest priority as the current thread
			}
			...
			curr = curr->next; // Setting the next thread as the current thread
		}
		enqueue(dequeueItem(queue, high_p->idx), &readyQ); // Enqueue the thread with the highest priority in the ready queue
		count--;
	}
	ENABLE();
}
 ...
 *  @Why : Once the tread finishes running its one instance , it must not vaish permanently
 * 			instead it should come back
 * 		: respawn_periodic_tasks bring those tasks back into the readyQ when their next period arrives
 */
void respawn_periodic_tasks(void)
{
	DISABLE();
	thread currentT = doneQ; // assuming the current thread as in the doneQ

	// restoring the thread until currentT is not null
	while (currentT != NULL)
	{
		...
		if (ticks % currentT->Rel_Period_Deadline == 0)
		{
			...
			enqueue(currentT, &readyQ); // Enqueue the current thread in the ready queue
			currentT = doneQ;			// Setting the current thread as the first thread in the done queue
			continue;
		}
		currentT = currentT->next; // Setting the next thread as the current thread
	}
	ENABLE();
}
